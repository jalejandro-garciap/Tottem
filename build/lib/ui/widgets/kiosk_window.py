...
class PaymentDialog(QDialog):
    def __init__(self, total_cents: int, parent=None):
        ...
        title = QLabel(f"{i18n.t('charge_total') or 'Total a cobrar'}")
        title.setObjectName("SectionTitle")
        title.setAlignment(Qt.AlignCenter)
        
        total_disp = QLabel(f"$ {self._fmt(self.total)}")
        total_disp.setObjectName("TotalLabel")
        total_disp.setAlignment(Qt.AlignCenter)
        
        root.addWidget(title)
        root.addWidget(total_disp)

        stats = QGridLayout()
        self.lbl_received = QLabel(f"{i18n.t('received') or 'Recibido'}: $ 0.00")
        self.lbl_change   = QLabel(f"{i18n.t('change') or 'Cambio'}: $ 0.00")
        stats.addWidget(self.lbl_received, 0, 0)
        stats.addWidget(self.lbl_change, 0, 1)
        root.addLayout(stats)

        row1 = QGridLayout(); row1.setSpacing(12)
        bills = [20, 50, 100, 200, 500, 1000]
        for idx, val in enumerate(bills):
            b = QPushButton(f"${val}")
            b.setMinimumHeight(64)
            b.clicked.connect(lambda _=None, v=val: self._add_bill(v))
            row1.addWidget(b, idx // 3, idx % 3)
        root.addLayout(row1)
...
class AdminPinDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.ed_pin.setMinimumHeight(64)
        self.ed_pin.setAlignment(Qt.AlignCenter)
        self.ed_pin.setStyleSheet("font-size: 24px;")
...
class POSWindow(QMainWindow):
    def __init__(self):
        ...
        # Left (categories/products) - Moved to be first in layout for better flow
        ...
        self.grid_wrap.setContentsMargins(16, 16, 16, 16)
        self.grid_wrap.setSpacing(12)
        ...
        self.btn_reprint.setIconSize(QSize(24, 24))
        ...
        self.btn_charge.setMinimumHeight(64)
        ...
        # Add to root with stretch factors for responsiveness
        root.addWidget(self.products_area, 3) # Product grid takes 3/5
        root.addWidget(self.right_wrap, 2)    # Cart takes 2/5
    ...
    def _target_button_min_size(self, avail_width: int) -> QSize:
        if avail_width <= 500: return QSize(140, 90)
        if avail_width <= 800: return QSize(160, 100)
        if avail_width <= 1200: return QSize(180, 110)
    ...
    def _calc_cols(self, avail_width: int, min_btn_w: int, spacing: int, margins: int) -> int:
        ...
        # Para pantallas muy grandes (22"), permitimos hasta 8 columnas si el ancho lo permite
        if avail_width > 1600:
            return min(cols, 8)
        return min(cols, 6)
